...
export default function SettingsPage() {
  ...
  const canonicalUsername =
    stats?.referral_username ||
    profile?.username ||
    profile?.Username ||
    profile?.referral_username ||
  ...
  const hasExistingUsername = profile
    ? Boolean(
        canonicalUsername &&
  ...
  const localPinHash =
    typeof window !== "undefined" && userEmail
      ? localStorage.getItem(`Axile_pin_hash_${userEmail}`)
      : null;
  ...
  const handleUsernameSubmit = async (e) => {
    ...
    try {
      ...
      console.error(
        "Settings: Username update failed",
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    if (!passData.old || !passData.new)
      return toast.error("Old and new passwords are required", toastTheme);
    if (passData.new !== passData.confirm)
      return toast.error("Passwords do not match", toastTheme);
    try {
  ...
  const handlePinSetup = async (e) => {
    ...
    if (pinData.pin.length !== 4)
      return toast.error("PIN must be exactly 4 digits", toastTheme);
    if (pinData.pin !== pinData.confirm)
      return toast.error("PINs do not match", toastTheme);
    ...
    <div className="max-w-2xl mx-auto pb-24 pt-8 px-4 sm:px-6 w-full relative z-10">
      ...
      >
        ...
        >
          <LogOut
            size={14}
            className="sm:mr-2 mr-0 group-hover:-translate-x-0.5 transition-transform"
          />
      ...
      >
        ...
        <div className="p-5 sm:p-7 relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p
                className={`text-[11px] sm:text-xs font-medium ${hasExistingUsername ? "text-emerald-400" : "text-white/40"}`}
              >
                {hasExistingUsername
          ...
          >
            ...
            <Button
              type="submit"
              disabled={isLoading || hasExistingUsername}
              className={`h-11 px-6 rounded-xl font-bold text-[11px] uppercase tracking-wider text-white transition-all w-full sm:w-auto shrink-0 shadow-sm ${hasExistingUsername ? "bg-white/5 text-white/30 cursor-not-allowed border border-white/5" : "bg-blue-600 hover:bg-blue-500"}`}
            >
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-3">
            ...
            <div className="pt-2 flex justify-end">
              <Button
                type="submit"
                disabled={isLoading}
                className="h-11 px-6 rounded-xl font-bold text-[11px] uppercase tracking-wider bg-rose-600 hover:bg-rose-500 text-white transition-all w-full sm:w-auto shadow-sm"
              >
                Change Password
              </Button>
            </div>
        ...
        <div className="p-5 sm:p-7">
          <div className="flex items-center gap-3 mb-4">
            <div
              className={`w-8 h-8 rounded-lg ${profile?.has_pin || profile?.pin_set || useAuthStore.getState().pinHash ? "bg-emerald-500/20 text-emerald-500" : "bg-emerald-500/10 text-emerald-500"} flex items-center justify-center shrink-0`}
            >
              <Fingerprint size={16} />
            </div>
            <div>
              ...
              <p
                className={`text-[11px] sm:text-xs font-medium ${hasExistingPin ? "text-emerald-400" : "text-white/40"}`}
              >
                {hasExistingPin
          ...
          {hasExistingPin ? (
            <div className="text-center py-6 px-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
              ...
              <p className="text-xs text-white/50 mt-1">
                Your security PIN has been set and locked.
              </p>
            </div>
          ) : (
            <form onSubmit={handlePinSetup} className="flex flex-col gap-6">
              ...
              <div className="flex justify-end">
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="h-11 px-8 rounded-xl font-bold text-[11px] uppercase tracking-wider bg-emerald-500 hover:bg-emerald-400 text-white transition-all w-full sm:w-auto shadow-sm"
                >
                  Set PIN
                </Button>
              </div>
